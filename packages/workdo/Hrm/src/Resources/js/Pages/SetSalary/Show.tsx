...
import CreateEmployerContribution from './EmployerContributions/Create';
import EditEmployerContribution from './EmployerContributions/Edit';
...
export default function Show() {
    ...
    const { employee, allowanceTypes, allowances: initialAllowances, deductionTypes, deductions: initialDeductions, employerContributionTypes, employerContributions: initialEmployerContributions, loanTypes, loans: initialLoans, overtimes: initialOvertimes, auth } = usePage<SetSalaryShowProps>().props;
    ...
    const [employerContributions, setEmployerContributions] = useState<EmployerContribution[]>(initialEmployerContributions || []);
    ...
    useEffect(() => {
        ...
        setEmployerContributions(initialEmployerContributions || []);
        setLoans(initialLoans || []);
        setOvertimes(initialOvertimes || []);
    }, [initialAllowances, initialDeductions, initialEmployerContributions, initialLoans, initialOvertimes]);
    ...
    const [employerContributionModalState, setEmployerContributionModalState] = useState<{
        isOpen: boolean;
        mode: string;
    ...
    const { deleteState: employerContributionDeleteState, openDeleteDialog: openEmployerContributionDeleteDialog, closeDeleteDialog: closeEmployerContributionDeleteDialog, confirmDelete: confirmEmployerContributionDelete } = useDeleteHandler({
        routeName: 'hrm.employer-contributions.destroy',
        defaultMessage: t('Are you sure you want to delete this employer contribution?')
    });
    ...
    const openEmployerContributionModal = (mode: string, data: EmployerContribution | null = null) => {
        setEmployerContributionModalState({ isOpen: true, mode, data });
    };

    const closeEmployerContributionModal = () => {
        setEmployerContributionModalState({ isOpen: false, mode: '', data: null });
    };
    ...
    return (
        ...
        >
                ...
                <Card className="shadow-sm">
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            ...
                            <Button size="sm" onClick={() => openEmployerContributionModal('add')}>
                                <Plus className="h-4 w-4" />
                            </Button>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <div className="overflow-x-auto max-h-80 overflow-y-auto">
                            <DataTable
                                ...
                                columns={[
                                    {
                                        ...
                                        sortable: false,
                                        render: (_: any, row: EmployerContribution) => row.employer_contribution_type?.name || '-'
                                    },
                                    {
                                        key: 'type',
                                        header: t('Type'),
                                        sortable: false,
                                        render: (_: any, row: EmployerContribution) => t(row.type === 'fixed' ? 'Fixed' : 'Percentage')
                                    },
                                    {
                                        key: 'amount',
                                        header: t('Amount'),
                                        sortable: false,
                                        render: (_: any, row: EmployerContribution) => {
                                            if (row.type === 'fixed') {
                                                return formatCurrency(row.amount) || '0';
                                            } else {
                                                return `${row.amount || '0'}%`;
                                    ...
                                    },
                                    {
                                        key: 'actions',
                                        header: t('Actions'),
                                        render: (_: any, contribution: EmployerContribution) => (
                                            <div className="flex gap-1">
                                                <TooltipProvider>
                                                    <Tooltip delayDuration={0}>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => openEmployerContributionModal('edit', contribution)}
                                                                className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700"
                                                            >
                                                                <Edit className="h-4 w-4" />
                                                            </Button>
                                                    ...
                                                    <Tooltip delayDuration={0}>
                                                        <TooltipTrigger asChild>
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => openEmployerContributionDeleteDialog([contribution.id])}
                                                                className="h-8 w-8 p-0 text-red-600 hover:text-red-700"
                                                            >
                                                                <Trash2 className="h-4 w-4" />
                                                            </Button>
                                ...
                                emptyState={
                                    <div className="text-center py-8">
                                        <p className="text-muted-foreground">{t('No employer contributions found')}</p>
                                    </div>
                                }
                            />
                        </div>
            ...
            {/* Employer Contribution Modals */}
            <Dialog open={employerContributionModalState.isOpen} onOpenChange={closeEmployerContributionModal}>
                {employerContributionModalState.mode === 'add' && (
                    <CreateEmployerContribution
                        employeeId={employee.user?.id || employee.id}
                        employerContributionTypes={employerContributionTypes}
                        onSuccess={closeEmployerContributionModal}
                    />
                )}
                {employerContributionModalState.mode === 'edit' && employerContributionModalState.data && (
                    <EditEmployerContribution
                        employerContribution={employerContributionModalState.data}
                        employerContributionTypes={employerContributionTypes}
                        onSuccess={closeEmployerContributionModal}
                    />
                )}
            ...
            <ConfirmationDialog
                open={employerContributionDeleteState.isOpen}
                onOpenChange={closeEmployerContributionDeleteDialog}
                title={t('Delete Employer Contribution')}
                message={employerContributionDeleteState.message}
                confirmText={t('Delete')}
                onConfirm={confirmEmployerContributionDelete}
                variant="destructive"
            />
        </AuthenticatedLayout>
    );
