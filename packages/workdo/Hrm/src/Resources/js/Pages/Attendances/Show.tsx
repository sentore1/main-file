...
import { Clock, User, Calendar, Timer, FileText, CheckCircle, Cpu } from 'lucide-react';
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div className="mt-1">
                            {(() => {
                                const src = attendance.source || 'web';
                                const sourceColors: Record<string, string> = {
                                    web: 'bg-blue-100 text-blue-800',
                                    mobile: 'bg-purple-100 text-purple-800',
                                    fingerprint: 'bg-orange-100 text-orange-800',
                                    card: 'bg-teal-100 text-teal-800',
                                };
                                const sourceIcons: Record<string, string> = {
                                    web: '🌐', mobile: '📱', fingerprint: '🖐', card: '💳',
                                };
                                return (
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${sourceColors[src] || 'bg-gray-100 text-gray-800'}`}>
                                        {sourceIcons[src]} {t(src.charAt(0).toUpperCase() + src.slice(1))}
                                    </span>
                                );
                            })()}
                        </div>
                    ...
                    {attendance.device_id && (
                        <div>
                            ...
                            <p className="mt-1 font-medium">{attendance.device_id}</p>
                        </div>
                    )}
                </div>
            </div>
