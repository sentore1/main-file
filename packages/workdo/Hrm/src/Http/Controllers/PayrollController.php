...
class PayrollController extends Controller
{
        ...
        if (Auth::user()->can('view-payrolls')) {
            ...
            $payroll->load(['payrollEntries' => function ($query) {
                if (Auth::user()->can('view-any-payrolls')) {
                    ...
                    $query->with(['employee.user', 'employee.branch'])->where('employee_id', Auth::id());
                } else {
                    $query->whereRaw('1 = 0');
                }
            }]);
    ...
    private function processEmployeePayroll($payroll, $employee, $workingDaysCount, $startDate, $endDate)
    {
        ...
        // Calculate allowances, deductions, overtimes, loans and employer contributions
        ...
        $employerContributionData = $this->calculateEmployerContributions($employee, $basicSalary);
        ...
        $employerContributionsBreakdown = $employerContributionData['breakdown'];
        $totalEmployerContributions = $employerContributionData['total'];
        ...
        PayrollEntry::create([
            ...
            'employer_contributions_breakdown' => $employerContributionsBreakdown,
    ...
    private function calculateEmployerContributions($employee, $basicSalary)
    {
        $contributions = EmployerContribution::with('employerContributionType')
            ->where('employee_id', $employee->user_id)
            ->where('created_by', creatorId())
            ->get();
        ...
        foreach ($contributions as $contribution) {
            $contributionType = $contribution->employerContributionType;
            $name = $contributionType->name ?? 'Employer Contribution';

            if ($contribution->type === 'percentage') {
                $amount = ($basicSalary * $contribution->amount) / 100;
            } else {
    ...
    {
        $payroll->load(['payrollEntries' => function ($query) {
            $query->with(['employee.user', 'employee.designation', 'employee.branch'])
                ->when(request('branch_id'), function($q) {
                    $q->whereHas('employee', function($eq) {
                        $eq->where('branch_id', request('branch_id'));
                    });
        ...
        return Inertia::render('Hrm/Payrolls/Export', [
            'payroll' => [
                'title' => $payroll->title,
                'pay_period_start' => $payroll->pay_period_start,
                'pay_period_end' => $payroll->pay_period_end,
                'payrollEntries' => $payroll->payrollEntries,
            ],
        ]);
    }
    ...
    {
        try {
            // Load payroll entries with branch filter
            $payroll->load(['payrollEntries' => function ($query) {
                $query->with(['employee.user', 'employee.designation', 'employee.branch'])
                    ->when(request('branch_id'), function($q) {
                        $q->whereHas('employee', function($eq) {
            ...
            // Set title
            $sheet->setCellValue('A1', 'PAYROLL STAFF EAST GATE HOTEL ' . strtoupper(date('F Y', strtotime($payroll->pay_period_start))));
            $sheet->mergeCells('A1:S1');
            $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
            $sheet->getStyle('A1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

            // Headers
            $headers = ['No', 'NAMES', 'Post', 'Basic salary', 'TRANSPORT', 'Gross salary', 
                        'PENSION 6%', 'MATERNITY 0.3%', 'PENSION 6%', 'O.H 2%', 
                        'MATERNITY 0.3%', 'Y:o.3%', 'MATERNITY 0.3%', 'Y:o.3%',
                        'TPR', 'CBHI', 'CREDIT', 'ADVANCE', 'NET SALARY'];
            ...
            $sheet->getStyle('A3:S3')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('90EE90');
            $sheet->getStyle('A3:S3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

            // Yellow columns for employer contributions
            $yellowColumns = ['I', 'J', 'M', 'N'];
            foreach ($yellowColumns as $colLetter) {
                $sheet->getStyle($colLetter . '3')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('FFFF00');
            }

            // Data rows
            $row = 4;
            foreach ($payroll->payrollEntries as $index => $entry) {
                $transport = $entry->allowances_breakdown['Transport'] ?? 0;
                $pensionEmployee = $entry->deductions_breakdown['Pension Contribution'] ?? ($entry->basic_salary * 0.06);
                $maternityEmployee = $entry->deductions_breakdown['Maternity Contribution'] ?? ($entry->basic_salary * 0.003);
                $pensionEmployer = $entry->employer_contributions_breakdown['PENSION 6%'] ?? $entry->employer_contributions_breakdown['Employer Pension'] ?? ($entry->basic_salary * 0.06);
                $ohEmployer = $entry->employer_contributions_breakdown['O.H 2%'] ?? ($entry->basic_salary * 0.02);
                $maternityEmployer = $entry->employer_contributions_breakdown['MATERNITY 0.3%'] ?? $entry->employer_contributions_breakdown['Employer Maternity'] ?? ($entry->basic_salary * 0.003);
                $tpr = $entry->deductions_breakdown['TPR'] ?? 0;
                $cbhi = $entry->deductions_breakdown['CBHI'] ?? 0;

                $sheet->setCellValue('A' . $row, $index + 1);
                $sheet->setCellValue('B' . $row, $entry->employee->user->name ?? '');
                $sheet->setCellValue('C' . $row, $entry->employee->designation->designation_name ?? '-');
                $sheet->setCellValue('D' . $row, $entry->basic_salary);
                $sheet->setCellValue('E' . $row, $transport);
                $sheet->setCellValue('F' . $row, $entry->gross_pay);
                $sheet->setCellValue('G' . $row, $pensionEmployee);
                $sheet->setCellValue('H' . $row, $maternityEmployee);
                $sheet->setCellValue('I' . $row, $pensionEmployer);
                $sheet->setCellValue('J' . $row, $ohEmployer);
                $sheet->setCellValue('K' . $row, $maternityEmployee);
                $sheet->setCellValue('L' . $row, $maternityEmployee);
                $sheet->setCellValue('M' . $row, $maternityEmployer);
                $sheet->setCellValue('N' . $row, $maternityEmployer);
                $sheet->setCellValue('O' . $row, $tpr);
                $sheet->setCellValue('P' . $row, $cbhi);
                $sheet->setCellValue('Q' . $row, 0);
                $sheet->setCellValue('R' . $row, 0);
                $sheet->setCellValue('S' . $row, $entry->net_pay);

                // Yellow background for employer contribution columns
                foreach ($yellowColumns as $colLetter) {
                    $sheet->getStyle($colLetter . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('FFFF00');
                }
            ...
            // Borders
            $sheet->getStyle('A3:S' . ($row - 1))->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
            ...
            // Generate file
            $fileName = 'PAYROLL_' . strtoupper(str_replace(' ', '_', $payroll->title)) . '_' . date('Y_m_d') . '.xlsx';
