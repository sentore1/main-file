...
export default function ReceiptModal({ isOpen, onClose, completedSale, globalSettings }: ReceiptModalProps) {
    ...
    const logoPath = logoUrl ? getImagePath(logoUrl) : '';
    ...
    const handleDownload = async () => {
        // Convert logo to base64 before downloading
        let logoBase64 = '';
        if (logoPath) {
            try {
        ...
        // Pass logo base64 to download function
        downloadReceiptPDF(completedSale, globalSettings, logoBase64);
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto backdrop-blur-none" aria-describedby="receipt-modal-description">
                    <DialogHeader className="no-print">
                        <DialogTitle className="flex items-center justify-center text-green-600">
                            <CheckCircle className="h-6 w-6 mr-2" />
                            {t('Sale Completed Successfully!')}
                        </DialogTitle>
                        <p id="receipt-modal-description" className="sr-only">{t('Your transaction receipt')}</p>
                    </DialogHeader>
                            ...
                            <div className="p-3">
                                ...
                                <div className="text-center mb-3">
                                    ...
                                    <div className="text-xs leading-relaxed">
                                        ...
                                        <div>{globalSettings?.company_city || 'City'}</div>
                                ...
                                <div className="mb-3">
                                    ...
                                    {completedSale.waiter_name && (
                                        <div className="flex justify-between py-0.5">
                                            <span className="font-medium">{t('Waiter')}:</span>
                                            <span className="truncate ml-2 font-medium">{completedSale.waiter_name}</span>
                                        </div>
                                    )}
                                </div>
                                    ...
                                    {completedSale.items.map((item: any) => {
                                        ...
                                        return (
                                            <div key={item.id} className="mb-3 pb-2 border-b border-dotted border-gray-300">
                                                ...
                                                {item.notes && (
                                                    <div className="text-xs text-blue-600 italic mb-1">📝 {item.notes}</div>
                                                )}
                                                {item.is_room && item.includes_breakfast && (
                                                    <div className="text-xs text-orange-600 font-medium mb-1">☕ {t('Includes Breakfast')}</div>
                                                )}
                                ...
                                <div className="mb-3">
                                    ...
                                    {completedSale.balance_due > 0 && (
                                        <>
                                            <div className="flex justify-between py-1 text-sm text-green-600">
                                                <span className="font-medium">{t('Paid')}:</span>
                                                <span className="font-bold">{formatCurrency(completedSale.paid_amount)}</span>
                                            </div>
                                            <div className="flex justify-between py-1 text-sm text-orange-600">
                                                <span className="font-medium">{t('Balance Due')}:</span>
                                                <span className="font-bold">{formatCurrency(completedSale.balance_due)}</span>
                                            </div>
                                        </>
                                    )}
                                </div>
