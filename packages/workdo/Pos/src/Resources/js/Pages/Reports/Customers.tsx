                                ...
                                <div className="w-full">
                                    <ResponsiveContainer width="100%" height={450}>
                                        <BarChart data={customerData?.slice(0, 10).map(customer => ({ 
                                            name: (customer.name || customer.customer?.name || 'Walk-in').length > 12 
                                                ? (customer.name || customer.customer?.name || 'Walk-in').substring(0, 10) + '...' 
                                                : (customer.name || customer.customer?.name || 'Walk-in'),
                                            fullName: customer.name || customer.customer?.name || 'Walk-in',
                                                    ...
                                                    if (active && payload && payload.length) {
                                                        return (
                                                            <div className="bg-white p-2 border rounded shadow-sm">
                                                                ...
                                                                <p className="text-sm text-blue-600">{formatCurrency(payload[0].value as number)}</p>
                                                            </div>
                                                        );
                                                    }
                                                    return null;
