        ...
        if(Auth::user()->can('manage-debit-notes')){
            try {
                $query = DebitNote::with(['vendor', 'purchaseReturn', 'approvedBy'])
                    ->where(function($q) {
                        if(Auth::user()->can('manage-any-debit-notes') || Auth::user()->can('manage-debit-notes')) {
                ...
                \Log::error('DebitNote index error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
                return back()->with('error', __('Failed to load debit notes: ') . $e->getMessage());
            }
        }
        else{
