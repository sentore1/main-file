...
Route::middleware(['web', 'auth', 'verified', 'PlanModuleCheck:ProductService'])->group(function () {
    ...
    Route::prefix('product-service/stock-reports')->name('product-service.stock-reports.')->group(function () {
        ...
        Route::get('/{date}/{type}/print', [\Workdo\ProductService\Http\Controllers\StockReportController::class, 'print'])->name('print');
        Route::delete('/{id}', [\Workdo\ProductService\Http\Controllers\StockReportController::class, 'destroy'])->name('destroy');
    });
});
