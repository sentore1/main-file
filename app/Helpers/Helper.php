...
if (!function_exists('creatorId')) {
    function creatorId()
    {
        // For non-SaaS: Always return company user ID
...
if (!function_exists('creatorUser')) {
    ...
    {
        // For non-SaaS: Always return company user or their creator
...
if (!function_exists('getAdminAllSetting')) {
    function getAdminAllSetting($publicOnly = false)
    {
        ...
        $settings = Cache::rememberForever($cacheKey, function () use ($publicOnly) {
            // For non-SaaS: Use company user instead of superadmin
            $company_user = User::where('type', 'company')->first();
            if ($company_user) {
                $query = Setting::where('created_by', $company_user->id);
                if ($publicOnly) {
        ...
        if (config('app.is_demo')) {
            ...
            $cookieName = 'theme_settings_' . ($company ? $company->id : 1);
    ...
    function getCompanyAllSetting($user_id = null, $publicOnly = false)
    {
        ...
        if ($user) {
            ...
            if (config('app.is_demo')) {
                ...
                $cookieName = 'theme_settings_' . $user->id;
                if (\Cookie::get($cookieName)) {
                    $cookieData = json_decode(\Cookie::get($cookieName), true);
                    if (is_array($cookieData)) {
                        foreach ($themeKeys as $cookieKey => $settingKey) {
...
if (!function_exists('ActivatedModule')) {
    ...
    {
        // For non-SaaS: Return all enabled modules
...
if (!function_exists('module_is_active')) {
    ...
    {
        // For non-SaaS: Just check if module exists and is enabled
...
if (!function_exists('assignPlan')) {
    function assignPlan($plan_id = null, $duration = null, $modules = null, $counter = null, $user_id = null)
    {
        ...
        */
        ...
        if ($user) {
            // For non-SaaS: Always set unlimited limits
            $user->total_user = -1;
            $user->storage_limit = -1;
            $user->plan_expire_date = null;
            $user->save();

            // Enable all users associated with this company
            $users = User::where('created_by', $user->id)->get();
            foreach ($users as $item) {
                $item->is_disable = 0;
                $item->is_enable_login = 1;
...
if (!function_exists('canCreateUser')) {
    function canCreateUser($userId = null)
    {
        ...
        */
        ...
        // For non-SaaS: Always allow user creation
        return ['can_create' => true];
    }
}
    ...
    function SetConfigEmail($user_id = null)
    {
        try {
            if (!empty($user_id)) {
                ...
                $user_id = User::where('type', 'company')->first()->id;
                $company_settings = getCompanyAllSetting($user_id);
            }
...
if (!function_exists('getUserWarehouseIds')) {
    function getUserWarehouseIds()
    {
        $user = Auth::user();
        
        // Super admin and company users see all warehouses
        if (in_array($user->type, ['super admin', 'company'])) {
            return \App\Models\Warehouse::where('created_by', creatorId())
                ->pluck('id')
                ->toArray();
        }
        ...
        // Add user's primary warehouse if assigned
        if ($user->warehouse_id) {
            $warehouseIds[] = $user->warehouse_id;
        }
        
        // Add additional assigned warehouses from user_warehouse table
        ...
        // If user has no warehouse assignments, return all warehouses (backward compatibility)
