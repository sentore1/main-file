...
class InstallerController extends Controller
{
    ...
    public function databaseStore()
    {
        try {
            ...
            $moduleList = implode(',', array_column($modules, 'name'));

            foreach ($modules as $module) {
                $this->enableModule($module['name']);
            }

            // Dispatch events to setup default data and permissions for all roles
            $company = User::where('type', 'company')->first();
            if ($company) {
                DefaultData::dispatch($company->id, $moduleList);

                $roles = ['staff', 'client', 'vendor','hr'];
                foreach ($roles as $roleName) {
                    $role = Role::where('name', $roleName)->where('created_by', $company->id)->first();
                    if ($role) {
