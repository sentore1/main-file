...
class PurchaseInvoiceController extends Controller
{
    ...
    {
        if(Auth::user()->can('print-purchase-invoices')){
            $query = PurchaseInvoice::with(['vendor', 'items.product', 'warehouse'])
            ...
            if ($request->filled('date_range')) {
                ...
                if (count($dateRange) == 2) {
                    $query->whereBetween('invoice_date', [$dateRange[0], $dateRange[1]]);
                }
            }
            ...
            return Inertia::render('Purchase/Report', [
                'invoices' => $invoices,
                'filters' => $request->only(['vendor_id', 'warehouse_id', 'status', 'date_range', 'search'])
            ]);
        }
        else{
            return back()->with('error', __('Permission denied'));
